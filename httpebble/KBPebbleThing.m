...
-(void) save
{
    ...
    [ips setFloat:lat forKey:@"lat"];
    [ips setFloat:lon forKey:@"lon"];
}
-(void)load
{
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    lat= [prefs floatForKey:@"lat"];
    lon = [prefs floatForKey:@"lon"];
...
{
    sleep(5);
    NSDictionary *response = @{HTTP_LOCATION_KEY: floatAsPBNumber(100.0f),
                               HTTP_LATITUDE_KEY: floatAsPBNumber(lat),
                               HTTP_LONGITUDE_KEY: floatAsPBNumber(lon),
                               HTTP_ALTITUDE_KEY: floatAsPBNumber(10.0f)
                               };
    NSLog(@"Sending location dictionary.%@",response);
